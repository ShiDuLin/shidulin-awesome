...
# 设置标准输出编码为 UTF-8（解决 Windows 终端中文和 emoji 显示问题）
...
def show_help():
    ...
    print("")
    print("用法:")
    print("  python3 scripts/fund.py invest <月定投额> <年化收益%> <定投年数>")
    print("      定投收益计算")
    print("  python3 scripts/fund.py dca <月定投额> <年化收益%> <定投年数>")
    print("      定投计算器（同invest，含复利+真实年化）")
    print("  python3 scripts/fund.py compare <金额> <年数> <收益率1%,收益率2%,...>")
    print("      多收益率对比")
    print("  python3 scripts/fund.py allocate <保守|稳健|激进> <总金额>")
    print("      资产配置建议（三种方案详细配置）")
    print("  python3 scripts/fund.py rebalance <当前持仓>")
    print("      再平衡建议（偏离目标配置时提醒调整）")
    print("  python3 scripts/fund.py types")
    print("      基金类型科普")
    print("  python3 scripts/fund.py strategy <保守|稳健|积极>")
    print("      定投策略建议")
    print("  python3 scripts/fund.py sharpe <年化收益%> <年化波动率%> <无风险利率%>")
    print("      夏普比率计算")
    print("  python3 scripts/fund.py maxdd <净值序列,逗号分隔>")
    print("      最大回撤分析")
    print("  python3 scripts/fund.py kelly <胜率%> <赔率(盈亏比)>")
    print("      凯利公式仓位计算")
    print("  python3 scripts/fund.py monte <月定投额> <年化收益%> <年化波动率%> <年数>")
    print("      蒙特卡罗模拟")
    print("  python3 scripts/fund.py fire <年支出> <年投资收益%> [当前存款]")
    print("      FIRE财务自由计算")
    print("  python3 scripts/fund.py var <投资总额> <年化收益%> <年化波动率%> [置信度%]")
    print("      VaR风险价值")
    print("  python3 scripts/fund.py irr <现金流序列,逗号分隔>")
    print("      内部收益率计算")
    print("  python3 scripts/fund.py cost <买入记录: 价格x份额,价格x份额,...>")
    print("      持仓成本分析")
    print("  python3 scripts/fund.py help")
    print("      显示帮助")
    print("")
    print("示例:")
    print("  python3 scripts/fund.py invest 2000 8 10")
    print("  python3 scripts/fund.py dca 3000 10 20")
    print("  python3 scripts/fund.py compare 1000 10 5,8,12")
    print("  python3 scripts/fund.py allocate 稳健 100000")
    print('  python3 scripts/fund.py rebalance "股票50000,债券30000,货币20000"')
    print("  python3 scripts/fund.py types")
    print("  python3 scripts/fund.py strategy 稳健")
...
def main():
    ...
    if cmd == 'help':
        ...
        if len(args) < 4:
            print("用法: python3 scripts/fund.py {} <月定投额> <年化收益%> <定投年数>".format(cmd))
            sys.exit(1)
        ...
        if len(args) < 4:
            print("用法: python3 scripts/fund.py compare <金额> <年数> <收益率1%,收益率2%,...>")
            sys.exit(1)
        monthly = float(args[1])
        years = int(args[2])
        rates_str = args[3]
        ...
        if len(args) < 2:
            print("用法: python3 scripts/fund.py strategy <保守|稳健|积极>")
            sys.exit(1)
        show_strategy(args[1])

    elif cmd == 'allocate':
        if len(args) < 3:
            print("用法: python3 scripts/fund.py allocate <保守|稳健|激进> <总金额>")
            sys.exit(1)
        allocate_assets(args[1], args[2])

    elif cmd == 'rebalance':
        if len(args) < 2:
            print("用法: python3 scripts/fund.py rebalance \"股票50000,债券30000,货币20000\"")
            sys.exit(1)
        rebalance_portfolio(args[1])

    elif cmd == 'sharpe':
        if len(args) < 4:
            print("用法: python3 scripts/fund.py sharpe <年化收益%> <年化波动率%> <无风险利率%>")
            print("示例: python3 scripts/fund.py sharpe 15 20 3")
            sys.exit(1)
        calc_sharpe(float(args[1]), float(args[2]), float(args[3]))

    elif cmd == 'maxdd':
        if len(args) < 2:
            print("用法: python3 scripts/fund.py maxdd <净值序列,逗号分隔>")
            print("示例: python3 scripts/fund.py maxdd 1.0,1.2,1.1,0.9,1.0,1.3")
            sys.exit(1)
        calc_max_drawdown(args[1])

    elif cmd == 'kelly':
        if len(args) < 3:
            print("用法: python3 scripts/fund.py kelly <胜率%> <赔率(盈亏比)>")
            print("示例: python3 scripts/fund.py kelly 60 2   (60%胜率，赚2倍亏1倍)")
            sys.exit(1)
        calc_kelly(float(args[1]), float(args[2]))

    elif cmd == 'monte':
        if len(args) < 5:
            print("用法: python3 scripts/fund.py monte <月定投额> <年化收益%> <年化波动率%> <年数>")
            print("示例: python3 scripts/fund.py monte 1000 10 15 20")
            sys.exit(1)
        calc_monte_carlo(float(args[1]), float(args[2]), float(args[3]), int(args[4]))

    elif cmd == 'fire':
        if len(args) < 3:
            print("用法: python3 scripts/fund.py fire <年支出> <年投资收益%> [当前存款]")
            print("示例: python3 scripts/fund.py fire 120000 6 500000")
            sys.exit(1)
        ...
        if len(args) < 4:
            print("用法: python3 scripts/fund.py var <投资总额> <年化收益%> <年化波动率%> [置信度%]")
            print("示例: python3 scripts/fund.py var 100000 8 15 95")
            sys.exit(1)
        ...
        if len(args) < 2:
            print("用法: python3 scripts/fund.py irr <现金流序列,逗号分隔>")
            print("示例: python3 scripts/fund.py irr -10000,3000,3000,3000,3000  (初始投1万,每年回3千)")
            sys.exit(1)
        calc_irr(args[1])

    elif cmd == 'cost':
        if len(args) < 2:
            print("用法: python3 scripts/fund.py cost <买入记录: 价格x份额,价格x份额,...>")
            print("示例: python3 scripts/fund.py cost 1.2x1000,1.0x2000,0.8x3000")
            sys.exit(1)
        calc_avg_cost(args[1])

    else:
        print("未知命令: {}".format(cmd))
        print("运行 'python3 scripts/fund.py help' 查看帮助")
        sys.exit(1)
